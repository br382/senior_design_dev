...
class Sensor():
    ...
    def work(self):
        ...
        self.data.out_json_obj               = {'update_user': self.data.users[self.myUID]}
    ...
    def _processHeading(self):
        try:
            ...
            deg = angle[1]
            self.data.users[self.myUID]['posStruct']['heading'] = deg
            if self.DEBUG: print 'heading ', self.data.users[self.myUID]['posStruct']['heading']
        except:
            pass
...
def main():
    ...
    sense = Sensor(port=m[0],baud=m[1], xbee_uid_range=xbees, DEBUG=True)
    while(1):
        print timestamp()
        sense.work()
