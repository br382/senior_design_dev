...
class Storage():
    def __init__(self):
        ...
        self.users        = {}
        self.errors       = []
        self.max_log_len  = 100
    def log(self, msg):
        if len(self.errors) > self.max_log_len:
            self.errors.pop(0)
        self.errors.append(t() + ' :: ' + str(msg))
        return
    ...
    def updateUser(self, update_user_obj, uid_old=None):
        ...
        if 1: #inttimestamp(source['last_modify']) > inttimestamp(self.users[uid_new]['last_modify']):
    ...
    def updateMarker(self, update_marker_obj, uid_old=None):
        ...
        if (uid_new != None):
            ...
            if 1: #inttimestamp(source['last_modify']) > inttimestamp(self.users[uid_new]['markerStruct']['last_modify']):
                for k in source.keys():
    ...
    def updatePos(self, update_pos_obj, uid_old=None):
        ...
        if (uid_new != None):
            ...
            print 'Cleaned: ' + str(source)
            if 1: #inttimestamp(source['last_modify']) > inttimestamp(self.users[uid_new]['posStruct']['last_modify']):
                for k in source.keys():
    ...
    def dump(self):
        ...
        with open(self.log_file, 'w') as f:
            f.write(dump_string)
        return None
    def load(self): #Use to load old `self.users` data
        filepath = self.log_file
        #check to verify proper datastruct is present:
        dump_struct = {
            "dump_timestamp"   :'',
            "self.in_json_obj" :'',
            "self.out_json_obj":'',
        ...
        #Verify loading valid datastruct
